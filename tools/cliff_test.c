/* Checks the "one cliff, one height" pass (vox_unify_solid_masses).
 *
 * The game marks a cell solid; the tile's colours decide how tall. That
 * vote is per 8x8 tile, so one cliff can come out ragged. This pass lets a
 * connected mass vote once. What matters is that it unifies what really is
 * one mass, and does NOT reach across gaps into a separate object.
 *
...
int main(void) {
    ...
    /* The LOW sits away from the wall: one touching a tall mass is meant
     * to lift, and has its own case further down. */
    clear_to(VOX_H_FLOOR);
    g[2][2] = VOX_H_WATER;
    g[2][3] = VOX_H_LOW;
    ...
    CHECK(g[2][3] == VOX_H_LOW, "a bevel with nothing tall beside it stays low");
    ...
    /* -- a diagonal inside a wall stops being a notch ----------------- */
    /* The real Seasons room 02-5E reads 0F 0F 0F 0C 0F 0F 0C 0F 0F 0F,
     * i.e. a tall wall with two partial-shape cells in the middle of it. */
    clear_to(VOX_H_FLOOR);
    for (int x = 2; x <= 11; x++) g[6][x] = VOX_H_HIGH;
    g[6][5] = VOX_H_LOW;                      /* the $0C diagonals */
    g[6][8] = VOX_H_LOW;
    vox_unify_solid_masses(g);
    CHECK(g[6][5] == VOX_H_MID && g[6][8] == VOX_H_MID,
          "a diagonal inside a tall wall becomes a bevel, not a hole");
    CHECK(g[6][4] == VOX_H_HIGH && g[6][9] == VOX_H_HIGH,
          "the wall around it stays tall");

    /* -- but a lip out in the open stays a lip ------------------------ */
    clear_to(VOX_H_FLOOR);
    g[8][3] = VOX_H_LOW;
    for (int x = 4; x <= 6; x++) g[8][x] = VOX_H_MID;   /* only rocks nearby */
    vox_unify_solid_masses(g);
    CHECK(g[8][3] == VOX_H_LOW,
          "a low bevel beside a mid mass is left as a bevel");
